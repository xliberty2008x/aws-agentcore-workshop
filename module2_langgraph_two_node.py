...
class AgentState(TypedDict):
    user_input: str
    route: NotRequired[Literal["tool", "direct", "clarify", "gateway"]]
    answer: str
    tool_called: bool
...
def route_node(state: AgentState):
    ...
    if "price" in text or "stock" in text:
        return {"route": "tool"}
    if any(k in text for k in ["weather", "погода"]):
...
def gateway_node(state: AgentState):
    text = state["user_input"].lower()
    location = "Kyiv"  # Default location for demo purposes
    weather_info = call_weather(location)
    return {"answer": f"The current weather in {location} is: {weather_info}", "tool_called": True}
...
def build_graph():
    ...
    g.add_node("gateway_node", gateway_node)

    g.add_edge(START, "route_node")
    g.add_conditional_edges("route_node", route_decision, {"tool_node": "tool_node", "direct_node": "direct_node", "clarify_node": "clarify_node", "gateway_node": "gateway_node"})
    g.add_edge("tool_node", END)
    g.add_edge("direct_node", END)
    g.add_edge("clarify_node", END)
    g.add_edge("gateway_node", END)
    return g.compile()
